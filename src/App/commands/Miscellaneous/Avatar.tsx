...
type AvatarCommandProps = {
  isDark: boolean;
};

const AvatarCommand = ({ isDark }: AvatarCommandProps) => {
  ...
  const badgeColor = isDark ? "bg-zinc-400 text-zinc-900" : "bg-zinc-900 text-zinc-200"; // Badge violet
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: avatar</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>miscellaneous</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Display the avatar of a specified user.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>user</code>: Mention the user whose avatar you want to view (Optional). Defaults to the command invoker if no user is mentioned.</li>
          </ul>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Required Permissions</h3>
          <p className="text-sm opacity-80">None.</p>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Usage Examples</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/avatar</code> – View your avatar.</li>
            <li><code>/avatar user:@Username</code> – View another user's avatar.</li>
          </ul>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Notes</h3>
          <p className="text-sm opacity-80">This command is useful for quickly accessing and viewing profile pictures.</p>
        </div>
