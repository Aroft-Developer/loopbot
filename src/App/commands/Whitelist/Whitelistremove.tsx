...
type WhitelistRemoveCommandProps = {
  isDark: boolean;
};

const WhitelistRemoveCommand = ({ isDark }: WhitelistRemoveCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: whitelist-remove</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>whitelist</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Remove a user from the whitelist, revoking their special access to the bot.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Subcommands</h3>
          <p className="text-sm opacity-80">/whitelist-remove – Remove a specific user from the whitelist.</p>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <p className="text-sm opacity-80">membre: Specify the user to be removed from the whitelist (Required).</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/whitelist-remove membre:@User</code> – Remove a user from the whitelist</li>
          </ul>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li>Users removed from the whitelist will be subject to the bot’s restrictions again.</li>
            <li>This command is useful for managing who has unrestricted access to the bot.</li>
...
export default WhitelistRemoveCommand;
