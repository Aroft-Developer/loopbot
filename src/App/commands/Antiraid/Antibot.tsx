...
type AntibotCommandProps = {
  isDark: boolean;
};

const AntibotCommand = ({ isDark }: AntibotCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: antibot</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>antibot</span>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/antibot enable</code>: Activates the antibot system.</li>
            <li><code>/antibot disable</code>: Deactivates the antibot system.</li>
          </ul>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <p className="text-sm opacity-80 mb-1">• <strong>status</strong>: Specify whether to enable or disable the antibot (Required).</p>
        ...
        <div className="mb-4">
          ...
          <p className="text-sm opacity-80">antibot</p>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Usage Examples</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/antibot status:enable</code> – Enable the antibot system</li>
            <li><code>/antibot status:disable</code> – Disable the antibot system</li>
          </ul>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li>The antibot system helps protect your server from unauthorized bot entries.</li>
            <li>Only users with administrative privilege can modify the antibot status.</li>
            <li>Errors in command usage or low permission level can notify even when the system is enabled.</li>
          </ul>
        </div>
