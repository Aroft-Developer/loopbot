...
type LockCommandProps = {
  isDark: boolean;
};

const LockCommand = ({ isDark }: LockCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: lock</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>moderation</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Lock a channel to prevent messages from being sent.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><strong>channel</strong>: The channel to lock (Required)</li>
            <li><strong>role</strong>: The role to restrict in the channel (Optional)</li>
          </ul>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Required Permissions</h3>
          <p className="text-sm opacity-80">You need the <code>ManageChannels</code> permission to use this command.</p>
        </div>
