...
type WarnListCommandProps = {
  isDark: boolean;
};

const WarnListCommand = ({ isDark }: WarnListCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: warnlist</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>moderation</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Display a list of all warnings issued to a specified user.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <p className="text-sm opacity-80">user: Mention the user whose warnings you want to view (Required).</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/warnlist user:@Username</code> – View warnings for a user</li>
          </ul>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li>The warnlist command provides insight into a user's warning history for better moderation decisions.</li>
          </ul>
        </div>
