...
const ClearCommand = ({ isDark }: ClearCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: clear</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>moderation</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Delete messages from a specific member or channel.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><strong>number</strong>: The number of messages to delete (max 100) (Required)</li>
            <li><strong>member</strong>: The member whose messages will be deleted (Optional)</li>
            <li><strong>channel</strong>: The channel where messages will be deleted (Optional)</li>
          </ul>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Required Permissions</h3>
          <p className="text-sm opacity-80">You need the <code>ManageMessages</code> permission to use this command.</p>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Category</h3>
          <p className="text-sm opacity-80">MODERATION</p>
        </div>
