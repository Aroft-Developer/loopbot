...
type MuteListCommandProps = {
  isDark: boolean;
};

const MuteListCommand = ({ isDark }: MuteListCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: mute-list</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>moderation</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Display a list of all currently muted users in the server.
        </p>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/mute-list</code> – View muted users</li>
          </ul>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li>This command is useful for monitoring moderation actions and ensuring compliance with server rules.</li>
          </ul>
        </div>
...
export default MuteListCommand;
