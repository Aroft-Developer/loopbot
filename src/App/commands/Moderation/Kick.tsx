...
type KickCommandProps = {
  isDark: boolean;
};

const KickCommand = ({ isDark }: KickCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: kick</h1>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><strong>member</strong>: The member to kick (Required)</li>
            <li><strong>reason</strong>: The reason for the kick (Optional)</li>
          </ul>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Required Permissions</h3>
          <p className="text-sm opacity-80">You need the <code>KickMembers</code> permission to use this command.</p>
        </div>
