...
type MuteCommandProps = {
  isDark: boolean;
};

const MuteCommand = ({ isDark }: MuteCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: mute</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>moderation</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Mute a member to prevent them from speaking or writing in the server.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><strong>member</strong>: The member to mute <em>(Required)</em></li>
            <li><strong>time</strong>: Duration of the mute <em>(Optional)</em></li>
            <li><strong>reason</strong>: The reason for the mute <em>(Optional)</em></li>
          </ul>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Permissions</h3>
          <p className="text-sm opacity-80">You need <code>ManageRoles</code> permission to use this command.</p>
        </div>
