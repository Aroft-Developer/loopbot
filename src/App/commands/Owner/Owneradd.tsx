...
type OwnerAddCommandProps = {
  isDark: boolean;
};

const OwnerAddCommand = ({ isDark }: OwnerAddCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: owner-add</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>owner</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Add members as owners of the bot, granting them special privileges.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Subcommands</h3>
          <p className="text-sm opacity-80">/owner-add – Add a specific member as an owner of the bot.</p>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Options</h3>
          <p className="text-sm opacity-80">membre: Specify the member to be added as an owner (Required).</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/owner-add membre:@User</code> – Add a member as an owner</li>
          </ul>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li>Members added as owners receive special privileges, allowing them to perform administrative actions beyond normal user permissions.</li>
            <li>Ensure the specified member is valid and exists on the server before executing the command.</li>
            <li>Use this command with caution, as it grants elevated access to the bot.</li>
          </ul>
        </div>
