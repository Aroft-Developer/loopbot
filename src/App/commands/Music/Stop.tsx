...
type StopCommandProps = {
  isDark: boolean;
};

const StopCommand = ({ isDark }: StopCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: stop</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>music</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Stop the currently playing music and clear the playback queue.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Subcommands</h3>
          <p className="text-sm opacity-80">/stop – Immediately stop the music playback.</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <p className="text-sm opacity-80">EVERYONE: All users can stop the music.</p>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Usage Examples</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/stop</code> – Stop the currently playing track.</li>
          </ul>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li>The command stops the music and clears the queue, if any tracks are queued.</li>
            <li>The bot will leave the voice channel after stopping the music, depending on its configuration.</li>
          </ul>
        </div>
