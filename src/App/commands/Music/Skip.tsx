...
type SkipCommandProps = {
  isDark: boolean;
};

const SkipCommand = ({ isDark }: SkipCommandProps) => {
    ...
    <div className={`font-[Outfit] px-6 py-12 min-h-screen transition ${bgColor} mr-[17%] ml-[17%]`}>
      ...
      <div className={`rounded-xl p-6 shadow border ${borderColor} ${cardColor}`}>
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">Command: skip</h1>
          <span className={`text-xs font-semibold px-2 py-1 rounded ${badgeColor}`}>music</span>
        </div>

        <h2 className="text-xl font-semibold mb-2">Description</h2>
        <p className="text-sm opacity-80 mb-4">
          Skip to the next song in the playback queue.
        </p>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Subcommands</h3>
          <p className="text-sm opacity-80">/skip – Move to the next track in the queue.</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <p className="text-sm opacity-80">EVERYONE: All users can skip to the next song.</p>
        </div>

        <div className="mb-4">
          <h3 className="font-semibold mb-1">Usage Examples</h3>
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li><code>/skip</code> – Skip the currently playing track.</li>
          </ul>
        </div>
        ...
        <div className="mb-4">
          ...
          <ul className="list-disc list-inside text-sm opacity-80 space-y-1">
            <li>If there are no songs in the queue, the bot will stop playback after skipping.</li>
            <li>The bot will automatically play the next song in the queue if one is available.</li>
          </ul>
        </div>
